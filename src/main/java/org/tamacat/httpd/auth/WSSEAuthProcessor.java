...
public class WSSEAuthProcessor extends AbstractAuthProcessor {
	...
	protected String getPasswordDigest(WSSE wsse, AuthUser user) {
		...
		byte[] nonce = Base64.getDecoder().decode(wsse.getNonce().getBytes());
		...
		return new String(Base64.getEncoder().encode(getSHA1(digest))).replace("\r", "").replace("\n", "");
	}
	...
	protected String getNonce() {
		...
		return new String(Base64.getEncoder().encode(nonce.getBytes()));
	}
